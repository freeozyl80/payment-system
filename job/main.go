...
func objectString(obj interface{}) string {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		sb.WriteString(fmt.Sprintf("%s: %v", field.Name, value.Interface()))
		if i < val.NumField()-1 {
			sb.WriteString(", ")
		}
	}
...
func main() {
	...
	cfg.AddDefaultHeader("X-Finnhub-Token", "cs1m7qpr01qsperucqbgcs1m7qpr01qsperucqc0")
	finnhubClient := finnhub.NewAPIClient(cfg).DefaultApi

	//Earnings calendar
	earningsCalendar, _, err := finnhubClient.EarningsCalendar(context.Background()).From("2024-10-01").To("2024-10-07").Execute()
	fmt.Printf("%+v\n", earningsCalendar)
